...
# Script that solves the optimization problem of the REFINERIES
# 1. The depot positions is given (optimization_depots.py)
# 2. The x_ij are not variables anymore, they are given
# 3. The position of refineries are calculated
# 4. The transported biomass and pellets are calculated
...
DEPOT_SOLUTION_DATA = '002_Optimization/output/clustering/solution_20_08_2023_21_00_47.csv'
...
print("DEPOT SOLUTION DATA: {}".format(DEPOT_SOLUTION_DATA))
...
# Load DEPOTS solution data (x_ij indexes)
df_x = pd.read_csv(DEPOT_SOLUTION_DATA, index_col=0)
...
## USE THIS IF YOU WANT TO TRY CALCULATION OF x_ij BASED ON PROPOSED ONE'S NEIGHBORS
...
# The arcs go from all the HS to the proposed depots (HUGE REDUCTION)
...
# The arcs go from the proposed depots to all the refinery candidates (HUGE REDUCTION)
...
# Get the forecasted biomass for year 2018 and 2019 of all the positions
...
## UNCOMMENT THIS IF YOU WANT TO USE x_ij AS VARIABLES
...
# TOLERANCES ARE IMPORTANT (ask for more info)
logging.info("\nSET CONSTRAINTS")
...
if status == OptimizationStatus.OPTIMAL or status == OptimizationStatus.FEASIBLE:
    ...
    MODEL_NAME = f'model_{dt_string}.lp'
    OUTPUT_MODEL_PATH = os.path.join(OUT_SYNTH_DATA_PATH, "models", MODEL_NAME)
    OUTPUT_CSV_PATH = os.path.join(OUT_SYNTH_DATA_PATH, f'solution_{dt_string}.csv')
