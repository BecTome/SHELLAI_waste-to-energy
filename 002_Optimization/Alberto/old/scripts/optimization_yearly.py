...
d_matrix = d_matrix.values[:N, :N]

df_fc = pd.read_csv(os.path.join(SYNTH_DATA_PATH, FORECAST_FILE))
df_fc = df_fc.iloc[:N, :]

cap_b_j = 1000 # Maximum depot capacity
cap_p_j = 5000 # Maximum production capacity
...
b_18 = [m.add_var(name=f'b_2018_{i}_{j}', lb=0) for i in range(N) for j in range(N)]
print(f"Variables b_2018 go from {b_18[0].name} to {b_18[-1].name}")

b_19 = [m.add_var(name=f'b_2019_{i}_{j}', lb=0) for i in range(N) for j in range(N)]
print(f"Variables b_2019 go from {b_19[0].name} to {b_19[-1].name}")

p_18 = [m.add_var(name=f'p_2018_{i}_{j}', lb=0) for i in range(N) for j in range(N)]
print(f"Variables p_2018 go from {p_18[0].name} to {p_18[-1].name}")

p_19 = [m.add_var(name=f'p_2019_{i}_{j}', lb=0) for i in range(N) for j in range(N)]
print(f"Variables p_2019 go from {p_19[0].name} to {p_19[-1].name}")

x = [m.add_var(name=f'x_{j}', var_type=BINARY) for j in range(N)]
print(f"Variables x go from {x[0].name} to {x[-1].name}")

r = [m.add_var(name=f'r_{j}', var_type=BINARY) for j in range(N)]
...
for i in range(N):
    m += xsum(m.var_by_name(f'b_2018_{i}_{j}') for j in range(N)) <= d_bio_18[i]
    m += xsum(m.var_by_name(f'b_2019_{i}_{j}') for j in range(N)) <= d_bio_19[i]

for j in range(N):
    # 3-4. Can't transport more than storage limit
    m += xsum(m.var_by_name(f'b_2018_{i}_{j}') for i in range(N)) <= cap_b_j * x[j]
    m += xsum(m.var_by_name(f'b_2019_{i}_{j}') for i in range(N)) <= cap_b_j * x[j]
    m += xsum(m.var_by_name(f'p_2018_{i}_{j}') for i in range(N)) <= cap_p_j * r[j]
    m += xsum(m.var_by_name(f'p_2019_{i}_{j}') for i in range(N)) <= cap_p_j * r[j]
    ...
    m += xsum(m.var_by_name(f'b_2018_{i}_{j}')  - m.var_by_name(f'p_2018_{j}_{i}') for i in range(N)) <=\
          .001 * x[j]
    m += xsum(m.var_by_name(f'p_2018_{j}_{i}') - m.var_by_name(f'b_2018_{i}_{j}') for i in range(N)) <=\
          .001 * x[j]
    
    m += xsum(m.var_by_name(f'b_2019_{i}_{j}')  - m.var_by_name(f'p_2019_{j}_{i}') for i in range(N)) <=\
          .001 * x[j]
    m += xsum(m.var_by_name(f'p_2019_{j}_{i}') - m.var_by_name(f'b_2019_{i}_{j}') for i in range(N)) <=\
          .001 * x[j]
    
# 5. Number of depots should be less than or equal to 25.
m += xsum(x[j] for j in range(N)) <= n_depots

# 6. Number of refineries should be less than or equal to 5.
m += xsum(r[j] for j in range(N)) <= n_refineries

# 7. At least 80% of the total forecasted biomass must be processed by refineries each year
m += xsum(m.var_by_name(f'p_2018_{i}_{j}') for i in range(N) for j in range(N))\
    >= 0.8 * total_fc_18
m += xsum(m.var_by_name(f'p_2019_{i}_{j}') for i in range(N) for j in range(N))\
    >= 0.8 * total_fc_19
